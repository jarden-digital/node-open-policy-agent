...
const evalQueryBoolean = (args, callback) => {
  ...
  executeCommand(cmd, (err, res) => callback(err, JSON.parse(res).result[0].expressions[0].value))
}
...
const help = (callback) => {
  executeCommand(opaPath + 'help', callback)
}
...
const version = (callback) => {
  executeCommand(opaPath + 'version', callback)
}
